<template>
  ...
  <el-dialog v-dialogDrag 
    title="请输入批注内容"
    :visible.sync="defaultOptions.dialogVisible"
    width="30%"
  >
  <!-- <span>请输入批注内容</span> -->
 <textarea name="word" id="pizhu" cols="30" rows="10" style="width:100%" @input="getvalue('pizhu')"></textarea>
...
export default createComponent({
  ...
  setup (props, { emit }) {
   ...
   function addHotspot (e) {
      defaultOptions.dialogVisible = true
      ...
      hotspot2 =  { x: relativeX / width * 100, y: relativeY / height * 100 }
      // for (let i = 0; i < schema.length; i++) {
      //   const value = schema[i]
      //   let showUserWord = value.property=='Title'?'标题':value.property=='Message'?'内容':value.property
     
      //  const fill = this.textarea
      //  // prompt(`请输入 ${showUserWord}`, value.default)
      //   if (fill === null) {
      //     return
      //   }
  ...
  function getvalue(id){
    textarea= document.getElementById(id).value;
      console.log('word',textarea)
   }

  function cancel(){
     defaultOptions.dialogVisible = false
     document.getElementById('pizhu').value = ''
  }
  function sure(){
      const unWrappedConfig = isRef(config) ? config.value : config
      const schema = unWrappedConfig.schema
      for (let i = 0; i < schema.length; i++) {
        ...
        hotspot2[value.property] = fill
      }
      unWrappedConfig.data.push(hotspot2)
      defaultOptions.dialogVisible = false
       document.getElementById('pizhu').value = ''
    }
...
span.ui__vue_hotspot_overlay {
  position: absolute;
  /* background-color: rgba(0, 0, 0, 0.4);   ws*/  /**/ 
  top: 0px;
  left: 0px;
  cursor: pointer;
  /* display: none;  */
